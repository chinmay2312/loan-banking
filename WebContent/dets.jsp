...
function myFunction1(s){
	 var y = document.getElementById("banga");
	 y.value=s;
...
<tr><td>EMI Date</td><td><%=l.getEmiDate() %></td></tr>
<tr><td>Applied Date</td><td><%=l.getAppliedDate()%></td></tr>
<tr><td>Sanctioned Date</td><td><%=l.getSanctionedDate() %></td></tr>
...
<form method="post" action="transs.jsp">
<input type="submit" name=<%=l.getReq_id()%> value="View Payment History" onclick="myFunction(name)"> 
<input type="hidden" name="sanga" id="sanga">
</form>
<form method="get" action="payment.jsp">
<input type="submit" name=<%=l.getReq_id()%> value="Pay Emi" onclick="myFunction1(name)" >
<input type="hidden" name="banga" id="banga">
